...
const moodleLink = course => course.moodle ? `<a class="moodle-link" href="${esc(course.moodle)}" target="_blank" rel="noopener noreferrer">打开 Moodle <span aria-hidden="true">↗</span></a>` : '';

function openDetails(item,trigger) {
  lastDetailTrigger=trigger;
  const info=dateInfo(item.date);
  document.querySelector('#detailContent').innerHTML=`<div class="detail-accent" style="--course:${item.course.color}"></div><div class="detail-kicker"><span>${esc(item.course.code)}</span><b>SEM ${item.course.semester}</b></div><h2 id="detailTitle">${esc(item.title)}</h2><div class="detail-tags"><span>${esc(kindLabels[item.kind]||'讲课')}</span>${item.optional?'<span>可选班次</span>':''}${item.kind==='exam'?'<span>重要日期</span>':''}</div><dl><div><dt>日期</dt><dd>${info.year}年${info.label} · ${info.day}</dd></div><div><dt>时间</dt><dd>${item.start}–${item.end}</dd></div><div><dt>地点</dt><dd>${esc(item.venue||'待定')}</dd></div><div><dt>教师</dt><dd>${esc(item.teacher||'待定')}</dd></div></dl><section class="detail-course" style="--course:${item.course.color}"><small>课程信息</small><h3>${esc(item.course.title)}</h3><p>${esc(item.course.coordinator)}</p><p>${esc(item.course.mode)} · ${esc(item.course.venue)}</p>${item.course.assessment?`<p><strong>考核：</strong>${esc(item.course.assessment)}</p>`:''}${item.course.note?`<p class="detail-note">${esc(item.course.note)}</p>`:''}${moodleLink(item.course)}</section>`;
